...
const headings: Record<string, { thanks: string; items: string; shipping: string; total: string }> =
  {
    ...
    en: { thanks: "Thank you for your order!", items: "Items", shipping: "Shipping", total: "Total" },
    ru: { thanks: "Спасибо за заказ!", items: "Товары", shipping: "Доставка", total: "Итого" },
    pl: { thanks: "Dziękujemy za zamówienie!", items: "Produkty", shipping: "Dostawa", total: "Razem" },
  };
...
function orderEmailHtml(order: OrderWithItems): string {
  ...
  const rows = order.items
    .map(
      (i) =>
        `<tr><td style="padding:6px 12px 6px 0">${i.nameSnapshot} × ${i.quantity}</td><td style="padding:6px 0;text-align:right">${formatPrice(i.unitPriceCents * i.quantity, locale)}</td></tr>`,
    )
    .join("");
  const destination =
    order.terminalName ??
  ...
  return `<div style="font-family:Arial,sans-serif;max-width:560px;margin:0 auto;color:#1c1917">
  <h1 style="color:#3a4c29">${h.thanks}</h1>
  <p><strong>Nr. ${order.number}</strong></p>
  <h3>${h.items}</h3>
  <table style="width:100%;border-collapse:collapse">${rows}
    <tr><td style="padding:6px 12px 6px 0;border-top:1px solid #ddd">${h.shipping}</td><td style="padding:6px 0;text-align:right;border-top:1px solid #ddd">${formatPrice(order.shippingCents, locale)}</td></tr>
